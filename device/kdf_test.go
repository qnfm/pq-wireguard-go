...
func TestNativeKDF(t *testing.T) {
	tests := []KDFTest{
		{
			key:   "746573742d6b6579",
			input: "746573742d696e707574",
	...
	for _, test := range tests {
		...
		// var keys [][]byte
		for i := 0; i < 3; i++ {
			k := make([]byte, blake2s.Size)
			if _, err := io.ReadFull(hkdf, k); err != nil {
				panic(err)
			}
			ti[i] = [blake2s.Size]byte(k)
			// keys = append(keys, key)
		}
		// KDF3(&t0, &t1, &t2, key, input)
		t0s := hex.EncodeToString(ti[0][:])
		t1s := hex.EncodeToString(ti[1][:])
		t2s := hex.EncodeToString(ti[2][:])
		assertEquals(t, t0s, test.t0)
		assertEquals(t, t1s, test.t1)
		assertEquals(t, t2s, test.t2)
	}
	...
	for _, test := range tests {
		...
		var ti [blake2s.Size]byte
		if _, err := io.ReadFull(hkdf, ti[:]); err != nil {
			panic(err)
		}
		t0s := hex.EncodeToString(ti[:])
		fmt.Printf("t0s:%s\n t0:%s\n\n", t0s, test.t0)
		assertEquals(t, t0s, test.t0)
	}
}
